...
void setup() {
  ...
  bottomButtonY = height - 40;  //bottom rows of buttons
  //Buttons
  ...
  tutorialButton = new Button(gapCam, bottomButtonY-10, buttonlength*2, buttonheight*1.5, "Tutorial");   
  nextButton = new Button(centerXRect - buttonlength, bottomYRect - buttonheight, buttonlength, buttonheight, " Next ");
  pauseButton = new Button(75 + buttonlength+ buttonlength/2, 100 + buttonheight + gap, buttonlength, buttonheight, "Pause");
  playButton = new Button(75, 100 + buttonheight + gap, buttonlength, buttonheight, "Play");
  ...
  vol=cp5.addSlider("Volume")
    .setPosition(sliderXRect, 165) //position of the slider
    .setWidth(150)      
  ...
  tuto1= loadImage("tutorial01.png"); //load tutorial images
...
void draw() {
  ...
  if (starting) {
    ...
    //change background if backgr= true, 
    if (backgr) {
...
//shows the webcam image with the rectangle in the center
void showWebcam() {
  video.loadPixels();
...
//check if keys listed are pressed
void keyPressed() {
  if (key == 'S' || key== 's') {
    //sun
    rcircle=false;
...
//check if mouse is clicked and where 
void mouseClicked() {
  if (startButton.CheckClick()) {
    ...
    if (returnVal==JFileChooser.APPROVE_OPTION) {
      ...
      songname = cp5.addTextlabel("songname")
        ...
        .setPosition(sliderXRect, 200)
        .setColorValue(0xffffffff)
        .setFont(createFont("Arial", 12))
        ;
      player.isPlaying();
    }
  } else if (pauseButton.CheckClick()) {
    player.pause();
...
void progressbar() {
  float l = map(player.position(), 0, player.length(), 0, sliderWidth);
  fill(0, 51, 102);
  rect(sliderXRect, 250, l, sliderHeight);
  push();
  fill(220);
  textSize(9);
  text("Duration", sliderWidth + 25, 250 + 10); 
  pop();
}

//background art for the main menue source:
...
void playTutorial() {
  tuto1.resize(width - width/3 - gap, height - 100 - buttonheight);
  tuto2.resize(width - width/3 - gap, height - 100 - buttonheight);
...
//draw a city where the buildings are presented by the freq
void cityArt() {
  fill(fillBackground);
  ...
  for (int i=0; i<fft.specSize(); i++) {
    ...
    fill(currentColor);
    //widthI*10 = rectangles in the background 
    rect(widthI*10, gap, i, bottomYRect - gap -s*7);
...
void sunArt() {
  ...
  for (int i = 0; i < fft.specSize(); i +=5) {
    for (float ang = 0; ang <= 360; ang += 5) { 
      radius = fft.getBand(i)*3.5; //freq
      float rad = radians(ang); //angle
      
      //calculates how long the lines should be and where the lines end
...
//draws a circle shaped line with frequency lines + circle, that reacts to freq
void rotatingCircleArt() {
  fill(fillBackground);
  rect(width/3 - gap, gap, width - width/3 - gap, height - 100);
  int circleSize = 130; //circle size for line(...)
  ...
  for (int i=0; i<fft.specSize(); i+=4) {     
    ...
    float range = map(i, 0, fft.specSize(), 0, 2 * PI);
    //frequency 
    float freq = abs(fft.getBand(i))*10;
    ellipse(0, 0, freq/5 + circleSize, freq/5 + circleSize);
    line(sin(range) * (circleSize), cos(range) * (circleSize), sin(range) * (freq + circleSize), cos(range) * (freq + circleSize));
  }
  popMatrix();
}
...
//von Musterloesung Uebung1 mit kleinen Aenderungen
class Button {
  float _x;
  float _y;
  float _width;
